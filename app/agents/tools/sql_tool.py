# SQL 执行工具 —— 兼容 slug sqlite-query 的只读查询技能声明（激活后可见）
# 三层只读防护（层层兜底）：
...
def create_execute_sql_tool(db_path: str, datasource_runtime=None):
    """创建只读 SQL 执行工具；平台数据源优先，演示 SQLite 兼容兜底。"""

    @tool
    def execute_sql(sql: str, limit: int = DEFAULT_LIMIT) -> str:
        """在请求选中的数据源上执行只读 SELECT 查询，返回 JSON 结果。
        ...
        查表结构应先调用 schema_search；未选择平台数据源时兼容使用演示 SQLite。
        ...
        if selection is not None:
            ...
            try:
                payload = datasource_runtime.execute_sql(
                ...
                logger.info(f"SQL 校验失败: {e} | sql_sha256={_sql_fingerprint(sql)}")
                return f"SQL 校验失败: {e}"
        ...
        try:
            ...
            try:
                ...
                if not result.ok:
                    logger.info(f"SQL 校验失败: {result.error} | sql_sha256={_sql_fingerprint(sql)}")
