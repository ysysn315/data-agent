...
def _write_report(path, cases):
    path.write_text(
        json.dumps({"summary": {"total": len(cases), "correct": sum(c["correct"] for c in cases), "accuracy": 0.5},
                    "by_tag": {}, "cases": cases}, ensure_ascii=False),
        encoding="utf-8",
    )
...
def test_load_failed_cases_filters_and_requires_golden(tmp_path):
    ...
    _write_report(
        ...
        [
            ...
            {"id": "bad1", "question": "q2", "golden_sql": "SELECT 2", "pred_sql": "SELECT 22", "correct": False, "error": "结果不等价"},
...
def test_build_candidate_keeps_error_in_meta(tmp_path):
    ...
    case = {"id": "bad1", "question": "各州 GMV", "tags": ["JOIN", "TopN"], "golden_sql": "SELECT 1",
            "pred_sql": "SELECT 2", "error": "结果不等价", "correct": False}
...
def test_compare_reports_markdown(tmp_path):
    ...
    base_data["by_tag"] = {"JOIN": {"total": 1, "correct": 0, "accuracy": 0.0}, "单表": {"total": 1, "correct": 1, "accuracy": 1.0}}
    ...
    after_data["by_tag"] = {"JOIN": {"total": 1, "correct": 1, "accuracy": 1.0}, "单表": {"total": 1, "correct": 1, "accuracy": 1.0}}
    after.write_text(json.dumps(after_data, ensure_ascii=False), encoding="utf-8")

    md = compare(json.loads(base.read_text(encoding="utf-8")), json.loads(after.read_text(encoding="utf-8")), "base.json", "after.json")
