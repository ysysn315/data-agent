...
| 主题 | 当前可核验事实 | 不要说 |
|---|---|---|
| 测试 | `213 passed, 5 skipped`；19 个测试文件 | 所有外部集成都已真机跑通 |
| Skills | 5 个内置技能；阶段测量约 1183→150 tokens/请求 | 当前 5 个技能仍精确等于早期测量口径 |
| Text-to-SQL | 28 条；3 份可区分报告；最高 89.29%（25/28） | 4 模型、92.86%（26/28） |
...
> 我做了一个面向业务数据分析的 Agent 平台，把中文问题转化成业务上下文检索、SQL 生成校验、只读执行和结果解释。平台用 Skills/MCP 做能力按需加载，用 RAG 和轻量知识图谱补充业务知识，用 LangGraph 承载多步分析，并通过 Text-to-SQL 与 RAG 评测把效果量化。重点不是功能堆叠，而是每条链路都有安全边界、失败降级和可核验结果。

### 3 分钟版本

按下面顺序展开，不要从框架名开始背：

1. **问数主链路**：用户问题先获得 M-Schema、业务术语和 SQL 示例，再由模型生成 SQL；sqlglot 做 AST 校验，SQLite 只读模式执行，最后解释结果。
...
- M-Schema 给表列补充中文注释；SQLite 没有原生 comment，未命中的注释保持为空，不能编造。
...
**边界**：演示数据源是 SQLite，schema 当前以 6 张表全量返回；没有完成多方言和行列级权限，不能包装成通用企业 BI 引擎。

**高频追问**：

1. 为什么比较执行结果而不是 SQL 文本？——正确 SQL 写法不唯一，业务目标是数据正确。
2. 为什么 SQL 示例和术语做成工具？——命中内容会出现在调用轨迹中，比静默拼 prompt 更可观察。
3. AST 校验是否绝对安全？——不是；它负责语法和策略层，数据库 `mode=ro` 才是最后的写保护。
