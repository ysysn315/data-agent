...
| 主题 | 当前可核验事实 | 不要说 |
|---|---|---|
| 测试 | `238 passed, 5 skipped`；21 个测试文件 | 所有外部集成都已真机跑通 |
...
**边界**：主 Chat 已统一接线，但 BM25 只恢复最多 `RAG_BM25_MAX_DOCUMENTS` 个 chunk，重启后仍需从 Milvus 重建；本地 BGE reranker 是可选依赖，未安装时回退配置的 LLM/融合排序。DOCX 只读普通段落，Markdown 超长章节不会二次按长度切分，图谱也没有实体向量链接或 PPR。

**高频追问**：

1. 主 Chat 如何保证重启后仍能混合检索？——Milvus 保存 content/metadata，知识库单例首次初始化时由共享 VectorStore 分批恢复 BM25；BM25 是派生索引，超过上限时保留受控规模，稠密召回仍覆盖完整库。
