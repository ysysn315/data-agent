...
| 主题 | 当前可核验事实 | 不要说 |
|---|---|---|
| 测试 | `240 passed, 5 skipped`；21 个测试文件 | 所有外部集成都已真机跑通 |
...
**边界**：主 Chat 已统一接线，但 BM25 只恢复最多 `RAG_BM25_MAX_DOCUMENTS` 个 chunk，重启后仍需从 Milvus 重建；恢复与文档列表扫描是同步 Milvus 操作，但已放入工作线程，不占用事件循环；本地 BGE reranker 是可选依赖，未安装时回退配置的 LLM/融合排序。DOCX 只读普通段落，Markdown 超长章节不会二次按长度切分，图谱也没有实体向量链接或 PPR。
