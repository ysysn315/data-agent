...
- 自动化测试：`240 passed, 5 skipped`（本分支当前工作区）；跳过项为当前环境不可用的真实 Docker 容器与 Redis worker 集成测试。
...
| 层次 | 当前接线 | 能力边界 |
|---|---|---|
| 主 Chat | Agent 工具 → 知识库单例首次初始化恢复 BM25 → 查询改写/扩展 → Milvus + BM25 → RRF → 元数据过滤 → 可选重排 | Milvus 是持久化真相源；BM25 为受上限约束的进程内派生索引，恢复和文档全量扫描放到工作线程；BGE 本地模型需额外安装 |
