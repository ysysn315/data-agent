...
- **用户体系与工作空间（默认关闭）**：`AUTH_ENABLED=true` 一键启用 —— API Key（sha256 哈希存储 + 常量时间比较）、bootstrap 自动建管理员、三级守卫链（optional→login→admin）、保护清单单一事实源、技能按工作空间隔离；MCP 写口限 admin（stdio=命令执行面）
- **前端全功能**：对话 / 技能管理 / MCP 管理 / 任务中心（SSE 进度时间线 + 报告渲染）/ 知识图谱 SVG 可视化 / 示例与术语管理
...
| [app/core/IMPLEMENTATION-auth.md](app/core/IMPLEMENTATION-auth.md) | 鉴权与工作空间：守卫链、保护清单、bootstrap |
